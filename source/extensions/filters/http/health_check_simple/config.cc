...
private:
  Http::FilterFactoryCb createFilterFactoryFromProtoTyped(
    ...
    std::vector<Http::HeaderUtility::HeaderData> header_data;
    header_data.emplace_back(header_proto);

    return [header_data, &context](Http::FilterChainFactoryCallbacks& callbacks) -> void {
      callbacks.addStreamDecoderFilter(std::make_shared<HealthCheckSimpleFilter>(header_data, context));
    };
  }
};
