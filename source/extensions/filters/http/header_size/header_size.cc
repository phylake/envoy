...
void HeaderSize::initConfig() {
  ...
  const std::string& name = HttpFilterNames::get().HeaderSize;
...
Http::FilterHeadersStatus HeaderSize::decodeHeaders(Http::HeaderMap& headers, bool) {
  initConfig();
  ...
  if (headers.byteSize() > settings_->maxBytes()) {
    callbacks_->sendLocalReply(Http::Code::RequestHeaderFieldsTooLarge, "", nullptr);
...
Http::FilterTrailersStatus HeaderSize::decodeTrailers(Http::HeaderMap&) {
